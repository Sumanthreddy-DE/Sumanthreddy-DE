<h1 align="center">Hi, I'm Sumanth Reddy</h1>
<p align="center">
  <em>Computational Engineer | Machine Learning for Mechanics & Materials | CAD Automation</em>
</p>

---

### About Me

- Working on **Physics-Informed Neural Networks** for composite material interface identification
- Interested in applying **ML to computational mechanics, materials science, and engineering automation**
- Experience with **NX Open API** for CAD automation and data extraction workflows
- Background in **Mechanical Engineering** with a focus on numerical methods and optimization
- Fun fact: I am not an Otaku anymore, I am just a Weeb 😄

---

### Languages & Tools

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=flat&logo=mathworks&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)
![LaTeX](https://img.shields.io/badge/LaTeX-008080?style=flat&logo=latex&logoColor=white)
![Solidity](https://img.shields.io/badge/Solidity-363636?style=flat&logo=solidity&logoColor=white)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat&logo=html5&logoColor=white)

---

### Featured Projects

- **[PINN for Composite Interface Identification](https://github.com/Sumanthreddy-DE/PINN-for-composite-interface-identification)**
  <sub>Physics-Informed Neural Network to identify equivalent Extended Interface parameters from 3-Layer Interphase configurations in composite materials. Uses the CSA approach with Mori-Tanaka method.</sub>

- **[Peak Power Minimization](https://github.com/Sumanthreddy-DE/Peak-Power-Minimization)**
  <sub>Peak power minimization with multiple harmonic loads using optimization techniques.</sub>

- **[Predicting the Bulk Modulus of Inorganic Crystals](https://github.com/Sumanthreddy-DE/Predicting-the-Bulk-Modulus-of-Inorganic-Crystals)**
  <sub>Machine learning approach to predict the bulk modulus of inorganic crystals.</sub>

- **[Simulation of Diffusion using Finite Differences](https://github.com/Sumanthreddy-DE/Simulation-of-Diffusion-using-Finite-Differences)**
  <sub>Numerical simulation of diffusion processes using finite difference methods.</sub>

- **[Topology Optimization of a Reservoir](https://github.com/Sumanthreddy-DE/Topology-Optimization-of-a-Reservoir)**
  <sub>Topology optimization in reservoir engineering using Python.</sub>

- **[Feed-Forward Neural Network](https://github.com/Sumanthreddy-DE/Feed-Forward-Neural-Network)**
  <sub>Python template for building and training feed-forward neural networks from scratch.</sub>

- **[Named Entity Recognition](https://github.com/Sumanthreddy-DE/Named-Entity-Recognition)**
  <sub>Named Entity Recognition on synthetic text data using Python.</sub>

---

### Engineering & CAD Automation

- **[Export NX CAD Files to CREO](https://github.com/Sumanthreddy-DE/Export-NX-CAD-files-to-CREO)**
  <sub>Python tool to export NX CAD files to CREO format.</sub>

- **[NX Constraints Training](https://github.com/Sumanthreddy-DE/NX-Constraints-training)**
  <sub>NX Open API scripts for constraint-based CAD automation.</sub>

- **[Production Engineering Data Automation](https://github.com/Sumanthreddy-DE/Production-Engineering-Data-Automation)**
  <sub>TypeScript application for automating production engineering data workflows.</sub>

---

### GitHub Stats

<p align="center">
  <img src="https://github-readme-stats-omega-lemon.vercel.app/api?username=Sumanthreddy-DE&show_icons=true&theme=radical" alt="GitHub Stats" />
</p>
<p align="center">
  <img src="https://github-readme-stats-omega-lemon.vercel.app/api/top-langs/?username=Sumanthreddy-DE&layout=compact&theme=radical" alt="Top Languages" />
</p>

---

### Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/sumanthreddys)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:Sumanthreddy.settipalli@fau.de)
